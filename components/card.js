...
const card = ({
  ...
  const content = (
    <div
      className={`${contentClassnames} col-sm-12 ${isLeft &&
        'Card__col--is-left'}`}
    >
      <h4 dangerouslySetInnerHTML={{ __html: title }} className="Card__title" />
      <p
        dangerouslySetInnerHTML={{ __html: paragraph }}
        className={`Card__paragraph Card__paragraph--is-${image.id}`}
      />
    </div>
  )
  const graphic = (
    <div
      className={`${graphicClassnames} col-sm-12 ${`Card__col--is-${image.id}`}`}
    >
      {Image && (
