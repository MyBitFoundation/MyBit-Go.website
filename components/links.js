...
class Links extends React.Component {
  ...
  render() {
    ...
    const renderedSelection = isFooter
      ? icons.map(icon => {
          if (icon.id === 'youtube' && !isFooter) return null
          if (icon.id === 'reddit' && !isFooter) return null
          if (icon.id === 'kakaostory' && !isFooter) return null
          const Icon =
            icon.id === 'slack' ? (isFooter ? icon.el[0] : icon.el[1]) : icon.el
          return (
            <div
              key={icon.id}
              className={`Links__icon ${isFooter &&
                'Links__icon--is-footer'} ${!isFooter &&
                'Links__icon--is-flag'} Links__icon--is-${icon.id}`}
            >
              <a
    ...
    return (
      <div
        className={`Links ${isFooter && 'Links--is-footer'} ${!isFooter &&
          'Links--is-header'} ${isMobileMenuActive && 'Links--is-active'} `}
      >
