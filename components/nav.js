...
const nav = ({
  ...
  const listItems = [
    ...
    { label: 'roadmap' },
    { label: 'team' },
    { label: 'blog', link: 'https://medium.com/mybit-dapp' },
    {
      label: 'FAQ',
      link: 'https://github.com/MyBitFoundation/MyBit.io/tree/master/faqs'
    }
  ].map(listItem => {
    ...
    return (
      <li
        key={listItem.label}
        onClick={() => clickMenuOption(listItem.label)}
  ...
  return (
    <nav className="Nav">
      ...
      <span
        onClick={onHandleToggleMobileMenu}
        className={`Nav__mobile-menu ${isMobileMenuActive &&
          'Nav__mobile-menu--is-active'}`}
      >
        {mobileIcon}
      </span>
      <div
        className={`Nav__list-wrapper ${isMobileMenuActive &&
          'Nav__list-wrapper--is-active'}`}
      >
        {navList}
      </div>
    </nav>
